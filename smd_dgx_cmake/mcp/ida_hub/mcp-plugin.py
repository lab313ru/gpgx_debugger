...
class MCPServer:
    ...
    def _run_server(self):
        ...
        try:
            ...
            # SO_REUSEADDR means something different on Windows: it lets a
...
def disassemble_function(
    ...
    for ea in idautils.FuncItems(func.start_ea):
        ...
        segment: Optional[str] = idaapi.get_segm_name(seg) if seg else None

        label: Optional[str] = idc.get_name(ea, 0)
        if not label or (label == func_name and ea == func.start_ea):
            label = None

        comments: list[str] = []
        c: Optional[str] = idaapi.get_cmt(ea, False)
        if c:
            comments.append(c)
...
def refresh_decompiler_ctext(function_address: int):
    # No Hex-Rays for this processor (e.g. 68k): nothing to refresh.
    if not ida_hexrays.init_hexrays_plugin():
        return
    try:
        error = ida_hexrays.hexrays_failure_t()
