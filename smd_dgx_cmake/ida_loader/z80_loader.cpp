...
static void print_version() {
  // A modal box on every load was noise; the log line says the same thing.
  msg("Sega Genesis/Megadrive Z80 drivers loader v%s;\n"
      "Author: DrMefistO [Lab 313] <newinferno@gmail.com>.\n", LDR_VERSION);
}
...
static int idaapi accept_file(qstring* fileformatname, qstring* processor, linput_t* li, const char* filename) {
  ...
  // A Z80 driver dump is at most the 8K of sound RAM it lives in. Empty files
  // are rejected outright — the original accepted them and produced a database
  // with nothing in it.
  const int64 size = qlsize(li);
  if (size <= 0 || size > 0x2000) return 0;

  fileformatname->sprnt("%s", "Sega Mega Drive Z80 driver");
  if (processor != nullptr)
    *processor = "z80";           // preselect the right module in the dialog
