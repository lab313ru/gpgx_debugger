...
void RefreshGrid(HWND hWnd)
{
    ...
    SI = FindGrid((size_t)GetMenu(hWnd));
    if (BGHS[SI].EDITING)
    {
...
int DetermineDataType(char* data)
{
    ...
    for (j = 0; j < k; j++)
    {
        if (isalpha(tbuffer[j])) { ALPHA = TRUE; }
...
void DisplayColumn(HWND hWnd, int SI, int c, int offset, HFONT hfont, HFONT hcolumnheadingfont)
{
    ...
    SetCell(&BGcell, r, c);
    strcpy(buffer, "");
    SendMessage(hWnd, BGM_GETCELLDATA, (size_t)&BGcell, (size_t)buffer);
    ...
    while (r <= BGHS[SI].bottomvisiblerow)
    {
        ...
        else
        {
            ...
            HPEN hpen, holdpen;
            iProtection = (int)SendMessage(hWnd, BGM_GETPROTECTION, (size_t)&BGcell, 0);
        ...
        iDataType = (int)SendMessage(hWnd, BGM_GETTYPE, (size_t)&BGcell, 0);
        ...
        if (iDataType == 5) //user drawn graphic
        {
            ...
            wParam = MAKEWPARAM((size_t)GetMenu(hWnd), BGN_OWNERDRAW);
            SendMessage(GetParent(hWnd), WM_COMMAND, wParam, (size_t)&rect);
        }
...
void SetCurrentCellStatus(HWND hWnd, int SelfIndex)
{
    ...
    if (SendMessage(hWnd, BGM_GETPROTECTION, (size_t)&BGcell, 0))
    {
...
char GetASCII(WPARAM wParam, LPARAM lParam)
{
    ...
    result = ToAscii((UINT)wParam, (lParam >> 16) && 0xff, keys, &dwReturnedValue, 0);
...
void NotifyRowChanged(HWND hWnd, int SI)
{
    ...
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_ROWCHANGED);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_SELCHANGE);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyColChanged(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_COLCHANGED);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_SELCHANGE);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyEndEdit(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_EDITEND);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyDelete(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_DELETECELL);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyEditBegin(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_EDITBEGIN);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF1(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F1);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF2(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F2);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF3(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F3);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF4(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F4);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF5(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F5);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF6(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F6);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF7(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F7);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF8(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F8);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF9(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F9);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF10(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F10);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF11(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F11);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyF12(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_F12);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void NotifyCellClicked(HWND hWnd, int SI)
{
    ...
    LPARAM lParam;
    lParam = MAKELPARAM(BGHS[SI].cursorrow, BGHS[SI].cursorcol);
    wParam = MAKEWPARAM((size_t)BGHS[SI].gridmenu, BGN_CELLCLICKED);
    SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
}
...
void CloseEdit(HWND hWnd, int SI)
{
    ...
    cell.row = r;
    cell.col = c;
    SendMessage(hWnd, BGM_SETCELLDATA, (size_t)&cell, (size_t)BGHS[SI].editstring);
...
LRESULT CALLBACK GridProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    SelfIndex = FindGrid((size_t)GetMenu(hWnd));
    ...
    case WM_PAINT:
        ...
        if (GetFocus() == hWnd)
        {
            PostMessage(hWnd, BGM_DRAWCURSOR, (size_t)SelfIndex, 0);
        }
    ...
    case WM_SETTEXT:
    {
        ...
        if (strlen(BGHS[SelfIndex].title) > 0)
        {
            ...
            GetTextExtentPoint32(gdc, BGHS[SelfIndex].title, (int)strlen(BGHS[SelfIndex].title), &size);
            SelectObject(gdc, holdfont);
    ...
    case BGM_PROTECTCELL:
        ...
        {
            wParam = MAKEWPARAM((size_t)GetMenu(hWnd), BGN_OUTOFRANGE);
            lParam = 0;
            SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
            ReturnValue = -1;
            break;
        ...
        if (FindResult != LB_ERR)
        {
            ...
            SendMessage(BGHS[SelfIndex].hlist1, LB_GETTEXT, FindResult, (size_t)buffer);
            if ((BOOL)lParam)
            {
                buffer[10] = 'P';
            }
            ...
            SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, FindResult, (size_t)buffer);
        }
        else
        {
    ...
    case BGM_SETCELLDATA:
        ...
        FindResult = (int)SendMessage(BGHS[SelfIndex].hlist1, LB_ADDSTRING, 0, (size_t)buffer);
        ...
        {
            int j;
            j = (int)SendMessage(BGHS[SelfIndex].hlist1, LB_GETCOUNT, 0, 0);
            if (j > 0)
            {
                SendMessage(BGHS[SelfIndex].hlist1, LB_GETTEXT, j - 1, (size_t)buffer);
                buffer[5] = 0x00;
                j = atoi(buffer);
    ...
    case BGM_GETCELLDATA:
        ...
        if (FindResult != LB_ERR)
        {
            ...
            char tbuffer[1000];
            //it was found, get it
            SendMessage(BGHS[SelfIndex].hlist1, LB_GETTEXT, FindResult, (size_t)lParam);
            strcpy(tbuffer, (char*)lParam);
            k = (int)strlen(tbuffer);
            c = 0;
            for (j = 13; j < k; j++)
        ...
        if ((BGHS[SelfIndex].EDITABLE) && (BGHS[SelfIndex].rows > 0))
        {
...

            {
                ...
                if (wParam == 8) //backspace
                {
                    ...
                    {
                        int j;
                        j = (int)strlen(BGHS[SelfIndex].editstring);
    ...
    case WM_SETFOCUS:
        ...
        wParam = MAKEWPARAM((size_t)GetMenu(hWnd), BGN_GOTFOCUS);
        lParam = 0;
        SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
        {TEXTMETRIC tm;
        HDC hdc;
    ...
    case WM_KILLFOCUS:
        ...
        wParam = MAKEWPARAM((size_t)GetMenu(hWnd), BGN_LOSTFOCUS);
        lParam = 0;
        SendMessage(GetParent(hWnd), WM_COMMAND, wParam, lParam);
        RefreshGrid(hWnd);
    ...
    case WM_CREATE:
        ...
        BG_GridIndex = AddGrid((size_t)GetMenu(hWnd));
        ...
        {
            BGHS[BG_GridIndex].gridmenu = (size_t)GetMenu(hWnd);
            ...
            SendMessage(hWnd, WM_SETTEXT, 0, (size_t)lpcs->lpszName);
        }
        if (BG_GridIndex == -1)
        {
            DestroyWindow(hWnd);
...
int BinarySearchListBox(HWND lbhWnd, char* searchtext)
{
    ...
    if (lbcount < 12)
    {
        ...
        FindResult = (int)SendMessage(lbhWnd, LB_FINDSTRING, -1, (size_t)searchtext);
    ...
    //is it the head?
    SendMessage(lbhWnd, LB_GETTEXT, head, (size_t)headtext);
    headtext[9] = 0x00;
    ...
    //is it the tail?
    SendMessage(lbhWnd, LB_GETTEXT, tail, (size_t)tailtext);
    tailtext[9] = 0x00;
    ...
    while ((!FOUND) && ((tail - head) > 1))
    {
        ...
        SendMessage(lbhWnd, LB_GETTEXT, finger, (size_t)tbuffer);
        tbuffer[9] = 0x00;
